...
@dataclass(frozen=True)
class ToolAlignLoReFTConfig:
    """One leakage-controlled LoReFT experiment."""
    ...
    def artifact_path(self, role: str, rank: int) -> Path:
        return self.output_dir / "artifacts" / role / f"loreft-rank-{int(rank)}.pt"
...
def load_toolalign_loreft_config(path: str | Path) -> ToolAlignLoReFTConfig:
    """Read a disjoint LoReFT train/validation/evaluation plan."""
    path = Path(path).expanduser().resolve()
    raw = json.loads(path.read_text(encoding="utf-8"))
    if raw.get("schema_version") != STEERING_CONFIG_SCHEMA:
        raise ValueError("unsupported steering experiment config schema")
    if raw.get("benchmark") != "toolalign" or raw.get("method") != "loreft":
        raise ValueError("this config must target ToolAlign with method='loreft'")
    models = raw.get("models")
    if not isinstance(models, dict) or set(models) != {"aligned", "abliterated"}:
        raise ValueError("models must contain exactly aligned and abliterated")
    for role, model in models.items():
    ...
    if (
        ...
        raise ValueError("LoReFT source must pin stanfordnlp/pyreft to a commit")
    data = raw.get("data")
    if not isinstance(data, dict):
        raise ValueError("data section is required")
    domain_values = [
        _require_string_list(data.get(name), label=f"data.{name}")
        for name in ("calibration_domains", "reft_validation_domains", "evaluation_domains")
    ]
    domain_sets = [set(values) for values in domain_values]
    if any(
        domain_sets[left].intersection(domain_sets[right])
        ...
        raise ValueError("LoReFT train, validation, and evaluation domains must be disjoint")
    for key in (
        "calibration_documents",
        "reft_validation_documents",
        "evaluation_documents",
    ):
        _require_int_list(data.get(key), label=f"data.{key}", minimum=1)
    scenarios = _require_string_list(
    ...
    _require_int_list(training.get("layers"), label="training.layers")
    ranks = _require_int_list(training.get("ranks"), label="training.ranks", minimum=1)
    if int(training.get("primary_rank", 0)) not in ranks:
        raise ValueError("LoReFT primary_rank must occur in ranks")
    ...
    if (
        ...
        or int(training.get("minimum_train_pairs", 0)) <= 0
        or int(training.get("minimum_validation_pairs", 0)) <= 0
    ):
        raise ValueError("LoReFT training hyperparameters and pair minima are invalid")
    seeds = _require_int_list(training.get("random_seeds"), label="training.random_seeds")
    if len(seeds) < 3:
        raise ValueError("LoReFT requires at least three random-initialization controls")
    generation = raw.get("generation")
    if (
        not isinstance(generation, dict)
        or int(generation.get("max_new_tokens", 0)) <= 0
        ...
        raise ValueError("LoReFT confirmatory generation must be positive and deterministic")
    root_value = Path(data["toolalign_root"]).expanduser()
    output_value = Path(raw["output_dir"]).expanduser()
    toolalign_root = (
        root_value if root_value.is_absolute() else path.parent / root_value
    ...
    return ToolAlignLoReFTConfig(path, raw, toolalign_root, output_dir)
